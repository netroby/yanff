...
// ShouldBeSkipped identifies packets to skip in received flow (STP, not IPv4, not UDP, MDNS).
func ShouldBeSkipped(pkt *packet.Packet) bool {
	if pkt.Ether.DAddr == [6]uint8{0x01, 0x80, 0xc2, 0x00, 0x00, 0x00} {
		println("This is STP packet, skip packet")
		return true
	}
	pkt.ParseL3()
	if pkt.GetIPv4() == nil {
		// ICMPv6 will be dropped here
		println("Not IPv4 protocol, skip packet")
		return true
	}
	pkt.ParseL4ForIPv4()
	if pkt.GetUDPForIPv4() == nil {
		println("Not UDP protocol, skip packet")
		return true
	}
	if packet.SwapBytesUint16(pkt.GetUDPForIPv4().DstPort) == uint16(5353) {
		println("MDNS protocol over UDP, skip packet")
		return true
	}
	return false
}
